...
export class Dictionary<TKey, TValue> implements IEnumerable<IKeyValuePair<TKey, TValue>> {
    ...
    firstOrDefault(predicate?: (element: IKeyValuePair<TKey, TValue>) => boolean): IKeyValuePair<TKey, TValue> | null {
        return ArrayExtensions.firstOrDefault(this.innerArray, predicate);
    }

    lastOrDefault(predicate?: (element: IKeyValuePair<TKey, TValue>) => boolean): IKeyValuePair<TKey, TValue> | null {
    ...
    sum<TI>(predicate: (element: IKeyValuePair<TKey, TValue>) => TI): TI | null {
        if (!predicate) throw new Error("Predicate can not be null.");
    ...
    get(key: TKey): TValue {
        ...
        if (!pair) throw new Error("The given key was not present in the dictionary.");
    ...
    keyOf(value: TValue): TKey {
        ...
        if (!pair) throw new Error("The given value was not present in the dictionary.");

        return pair.key;
    }

    add(key: TKey, value: TValue): void {
        if (!key) throw new Error("Key can not be null.");

        if (this.containsKey(key)) throw new Error("An element with the same key already exists in the Dictionary<TKey, TValue>.");
    ...
    addRange(value: any): void {
        ...
        for (i = 0; i < value.length; i++) {
            ...
            if (!innerValue) throw new Error("Item can not be null");

            this.add(innerValue.key, innerValue.value);
        }
    }

    pop(): IKeyValuePair<TKey, TValue> | undefined {
