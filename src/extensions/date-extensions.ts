...
export class DateExtensions {
    ...
    static getTwoDigitYear(date: Date): string | null {
        const year = date.getFullYear();

        return year < 0 ? "-" + DateExtensions.getTwoDigit(Math.abs(year)) : DateExtensions.getTwoDigit(year);
    }

    static getTwoDigitUTCYear(date: Date): string | null {
    ...
    static fromIso8601(value: string): Date {
        ...
        try {
            ...
            if (d === null) throw new Error("Unable to parse date.");
    ...
    private static getTwoDigit(value: number): string | null {
        const str = value.toString();
        return str.length < 2 ? StringExtensions.padRight(str, 2, "0") ?? "" : str.substr(str.length - 2, 2);
    }
}
