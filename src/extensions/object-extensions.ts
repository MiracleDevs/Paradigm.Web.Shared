...
export class ObjectExtensions {
    ...
    static getTypeName(objectInstance: any): string {
        if (objectInstance === null || objectInstance === undefined) throw new Error("Object can not be null.");
        ...
        if (objectInstance.constructor != undefined) {
            ...
            const objType = typeof objectInstance;
            const casedObjectType = objType.charAt(0).toUpperCase() + objType.slice(1);
    ...
    static isNativeValueType(objectInstance: any): boolean {
        ...
        const name = ObjectExtensions.getTypeName(objectInstance).toLowerCase();
        return name === "string" || name === "number" || name === "boolean" || name === "date";
    }

    static isEqualTo(source: any, other: any, ignore?: Array<string>, checkObjectType = true): boolean {
        if (!source && !other) return true;

        if ((!source && other) || (source && !other)) return false;

        if (checkObjectType && ObjectExtensions.getTypeName(source) !== ObjectExtensions.getTypeName(other)) return false;

        if (
            (ObjectExtensions.getTypeName(source) === "Number" && ObjectExtensions.getTypeName(other) === "Number") ||
        ...
        if (source instanceof Array && other instanceof Array) {
            ...
            for (const index in sourceKeys) {
                if (Object.prototype.hasOwnProperty.call(sourceKeys, index)) {
                    const key = sourceKeys[index];

                    if (!Object.prototype.hasOwnProperty.call(source, key)) continue;
                    if (ignore && ArrayExtensions.contains(ignore, key)) continue;
    ...
    static getDifference(source: any, other: any, ignore?: Array<string>, checkObjectType = true): string | null {
        let difference: string | null = null;

        if (!source && !other) return null;
        if ((!source && other) || (source && !other)) return "different value";

        if (checkObjectType && ObjectExtensions.getTypeName(source) !== ObjectExtensions.getTypeName(other)) return "different type";
        ...
        if (source instanceof Array && other instanceof Array) {
            if (source.length !== other.length) return "different lengths";

            for (let arrayIndex = 0; arrayIndex < source.length; arrayIndex++) {
                difference = ObjectExtensions.getDifference(source[arrayIndex], other[arrayIndex], ignore, checkObjectType);

                if (difference && !StringExtensions.isNullOrEmpty(difference)) {
    ...
    static clone(object: any, ignore?: Array<string>): any {
        if (
            ObjectExtensions.getTypeName(object) === "Number" ||
        ...
        function clone(source: any, destination: any): void {
            ...
            for (const index in sourceKeys) {
                if (Object.prototype.hasOwnProperty.call(sourceKeys, index)) {
                    ...
                    if (!isArray && !Object.prototype.hasOwnProperty.call(source, key)) continue;
                    if (!isArray && ignore && ArrayExtensions.contains(ignore, key)) continue;
