...
export class ArrayExtensions
{
    ...
    static firstOrDefault<T>(array: Array<T>, predicate?: (element: T) => boolean): T
    {
        if (!predicate)
        {
            if (array.length > 0)
    ...
    static lastOrDefault<T>(array: Array<T>, predicate?: (element: T) => boolean): T
    {
        if (!predicate)
        {
            if (array.length > 0)
    ...
    {
        var element = ArrayExtensions.firstOrDefault(array, predicate);
    ...
    {
        var element = ArrayExtensions.lastOrDefault(array, predicate);
    ...
    {
        var elements = !predicate ? array.slice() : ArrayExtensions.where(array, predicate);
        var count = 0;
